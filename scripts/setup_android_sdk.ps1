...
try {
    ...
    if (Test-Path "$cmdlineToolsDir\temp\cmdline-tools") {
        ...
        Write-Host "Command line tools extracted"
    }
    ...
    Write-Host "⚠️  You may need to download manually from: https://developer.android.com/studio#command-tools"
}
...
try {
    ...
    Write-Host "Platform tools extracted"
...
if (Test-Path "$SdkPath\cmdline-tools\latest\bin\sdkmanager.bat") {
    ...
    foreach ($component in $sdkComponents) {
        ...
        try {
            & "$SdkPath\cmdline-tools\latest\bin\sdkmanager.bat" --sdk_root=$SdkPath $component --accept-licenses | Out-Null
            Write-Host "   ✅ $component installed"
        } catch {
            Write-Warning "   ⚠️  Failed to install $component"
        }
    }
} else {
    Write-Warning "⚠️  SDK Manager not found. You may need to install SDK components manually."
}
...
Write-Host "   • Build your Bahá'í Resource Library APK"
